...
def generate_consensus(forward_reads, reference_fasta, output_fasta, logfile='logfile.log',
...
if __name__ == '__main__':
    ...
    for pair in paired_reads:
        bait_and_trim(forward_reads=pair[0], reverse_reads=pair[1],
                      output_dir=os.path.join(args.output_directory, 'tmp'),
                      plasmid_db=args.plasmid_database, threads=args.threads)
        plasmids = mash_for_potential_plasmids(forward_reads=os.path.join(args.output_directory, 'tmp', 'plasmid_reads_R1.fastq.gz'),
                                               reverse_reads=os.path.join(args.output_directory, 'tmp', 'plasmid_reads_R2.fastq.gz'),
                                               output_dir=os.path.join(args.output_directory, 'tmp'),
        ...
        kmerize_individual_fastas(potential_plasmid_list=plasmids,
                                  fasta_dir=os.path.join(args.output_directory, 'tmp'),
                                  output_dir=os.path.join(args.output_directory, 'tmp'),
                                  threads=args.threads)
        plasmid_scores = find_plasmid_kmer_scores(kmc_database_dir=os.path.join(args.output_directory, 'tmp'),
                                                  reads_kmerized=os.path.join(args.output_directory, 'tmp', 'read_kmers'),
                                                  output_dir=os.path.join(args.output_directory, 'tmp'))
        filtered_plasmids = filter_similar_plasmids(plasmid_scores, os.path.join(args.output_directory, 'tmp'))
